...
- **Swift 6 language mode is on by default package-wide** (`Package.swift`'s `swiftSettings` passes `-swift-version 6` to the `ClaudeMonitor` target — kept as an `unsafeFlags` override rather than bumping `swift-tools-version` to 6.0, so the manifest stays parseable by any 5.9+ SwiftPM). A plain `swift build` therefore already surfaces every strict-concurrency diagnostic CI's toolchain would; local and CI agree by construction, so the local proxy build (`swift build -Xswiftc -swift-version -Xswiftc 6`) this note used to recommend is retired — there is nothing it catches that a normal `swift build` doesn't. Narrowest-correct annotations are used throughout: `@MainActor` for state genuinely only ever touched from the app's one main-thread call path (`UsageStore`, `OAuthPoller`, AppKit window-controller caches, the CLI/headless entry points that drive them), `Sendable`/`@unchecked Sendable` for classes with real internal synchronization or no mutable state (`FileLogger`, `AnthropicAPIClient`, `OpenAIAPIClient`), and `nonisolated`/`nonisolated(unsafe)` only for pure helpers or genuinely single-threaded accumulators (documented at each site) — never a blanket suppression.
