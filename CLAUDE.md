...
- `menubar-app/ClaudeMonitor/` - Swift Package Manager project
  ...
  - `Sources/CodexAppServer.swift` - `CodexAppServerClient`: reads Codex usage by spawning `codex -s read-only -a untrusted app-server` and speaking JSON-RPC over its stdio, so **no OpenAI credential is read, stored, or refreshed by this app**. The only place in the package that spawns a subprocess. Framing is **newline-delimited JSON** (not `Content-Length`), replies **omit the `jsonrpc` member**, notifications have no `id`, and `windowDurationMins` is **minutes** (× 60 for `RateLimitWindow`). `-32600`/`-32601` mean *capability absent → fall back*, never *account unhealthy* — codex 0.46.0 returns it for every unknown method; an unauthenticated `CODEX_HOME` returns it too, disambiguated by `account/read`'s `account == null`. Shutdown is stdin-close → SIGTERM → SIGKILL in a `defer`; never call `Process.waitUntilExit()` off the main thread (it spins a CFRunLoop and hangs). `codex` is resolved to an absolute path from a candidate list with a `CLAUDE_MONITOR_CODEX_BIN` override — `/usr/bin/env` fails in a Finder-launched bundle.
  - `Sources/CodexCLI.swift` - `claude-monitor codex import` (one-shot CLI, no `--headless` needed, works on Linux)
  - `Sources/OAuthPoller.swift` - Per-provider usage polling (Anthropic ping / OpenAI usage GET), token add/import, proactive OpenAI token refresh, Fable probes (Anthropic only). Absent windows are stored as NULL, not 0, for non-Anthropic providers. `pollOpenAI` is a three-tier ladder ordered by how little credential handling each rung needs: `codex app-server` → bearer read from `auth.json` at request time → the stored credential (with its refresh loop). A merely *unavailable* tier falls through silently; only the last tier's own failure sets a status.
